...
### Stack
- Language and version: Python 3.x, JavaScript (Node.js)
- Framework(s): FastAPI, React (Vite), Tailwind CSS, SQLModel
- Package manager: pip (backend), npm (frontend)
- Database: SQLite (meroshare.db)
- Runtime / deployment target: Local (uvicorn / vite)

### Commands
- Install: `pip install -r backend/requirements.txt && cd frontend && npm install`
- Run locally (Backend): `uvicorn backend.main:app --reload`
...
### Layout
- Source lives in: `backend/`, `frontend/src/`
- Tests live in: `backend/` (test files prefixed with `test_`)
- Do not modify: `node_modules`, `__pycache__`, `frontend/dist`

### Conventions specific to this repo
- Naming: PEP 8 for Python, camelCase for JavaScript
- Import style: Absolute imports preferred
- Error handling pattern: Standard Python exceptions in backend, error boundaries in frontend
- Testing pattern and framework: pytest (backend), Vitest (likely, for frontend)

### Forbidden
- No hardcoding of API keys or credentials.
- Do not perform "drive-by" refactors of unrelated files.
