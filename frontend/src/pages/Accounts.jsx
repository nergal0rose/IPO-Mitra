...
export default function Accounts() {
  ...
  const handleDelete = (id) => {
    ...
    api.delete(`/api/accounts/${id}`).then(() => { fetchAccounts(); toast.success('Deleted'); });
  };
    ...
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: '#6B7280', marginTop: 2 }}>Manage your MeroShare family accounts</div>
        </div>
      ...
      {showForm && (
        <div className="page-enter" style={{ background: '#111827', border: '1px solid #1F2937', borderRadius: 10, padding: '16px 14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            ...
            <button onClick={() => setShowForm(false)} style={{ color: '#6B7280', background: 'none', border: 'none', cursor: 'pointer' }}><X size={16} /></button>
          </div>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 5 }}>Capital (DP)</div>
            ...
            <div className="lg:col-span-3 pt-2 flex gap-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="btn" style={{ padding: '7px 14px', borderRadius: 7, fontSize: 12, fontWeight: 500, background: 'transparent', border: '1px solid #374151', color: '#9CA3AF', cursor: 'pointer' }}>Cancel</button>
            </div>
          </form>
        </div>
      )}
          ...
          return (
            <div key={acc.id} className="card" style={{ background: '#111827', border: '1px solid #1F2937', borderRadius: 9, padding: '12px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 500, flex: 1, color: '#F3F4F6' }}>{acc.name}</div>
                {hs && <StatusBadge label={isOk ? 'OK' : 'Fail'} variant={isOk ? 'success' : 'error'} />}
              </div>
              <div style={{ fontSize: 10, color: '#6B7280', fontFamily: "'JetBrains Mono', monospace", marginBottom: 3 }}>
                {acc.username}
              </div>
      ...
      {!loading && accounts.length === 0 && (
        <div style={{ background: '#111827', border: '1px dashed #1F2937', borderRadius: 10, padding: '32px 14px', textAlign: 'center' }}>
          <div style={{ fontSize: 12, color: '#6B7280' }}>No accounts yet. Add your MeroShare family accounts to get started.</div>
        </div>
      )}
    </div>
  );
