...
function StatCard({ label, value, valueColor, sub }) {
  return (
    <div style={{ background: '#111827', border: '1px solid #1F2937', borderRadius: 10, padding: '12px 14px' }}>
      <div style={{ fontSize: 11, color: 'rgba(255, 255, 255, 0.8)', marginBottom: 5 }}>{label}</div>
      <div style={{ fontSize: 20, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace", color: valueColor }}>{value}</div>
      <div style={{ fontSize: 10, color: 'rgba(255, 255, 255, 0.8)', marginTop: 3 }}>{sub}</div>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ flex: 1, paddingLeft: 4 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {closeDate && (
            <span style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: urgency === 'today' ? '#EF4444' : urgency === 'tomorrow' ? '#F59E0B' : 'rgba(255, 255, 255, 0.8)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255, 255, 255, 0.8)' }}>per share · min {ipo.minUnit || 10}</div>
        </div>
        <Link to="/apply" style={{
        ...
        <button style={{
          padding: '6px 10px', borderRadius: 6, background: 'transparent', color: 'rgba(255, 255, 255, 0.8)', fontSize: 11,
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 10, color: 'rgba(255, 255, 255, 0.8)' }}>{account.username}</div>
      </div>
      <div style={{ fontSize: 10, color: isOk ? 'rgba(255, 255, 255, 0.8)' : '#EF4444', textAlign: 'right', fontFamily: "'JetBrains Mono', monospace" }}>
    ...
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255, 255, 255, 0.8)', marginTop: 2 }}>{dateStr}</div>
        </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255, 255, 255, 0.8)' }}>{openIpos.length} active subscriptions</div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {!loading && openIpos.length === 0 && (
            <div style={{ background: '#111827', border: '1px dashed #1F2937', borderRadius: 10, padding: '32px 14px', textAlign: 'center' }}>
              <div style={{ fontSize: 12, color: 'rgba(255, 255, 255, 0.8)' }}>No open IPOs right now. Check back during subscription windows.</div>
            </div>
          )}
        </div>
      </div>
